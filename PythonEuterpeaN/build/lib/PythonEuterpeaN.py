...
# Python 3 map compatibility hack; can't just use "map" in most places where it was used with Py2
def listMap(fun, iter):
...
class Music:
    ...
    """
    def __init__(self, tree, bpm=120, params=None):
        self.tree = tree
        self.bpm = bpm
...
class Seq:
    ...
    """
    def __init__(self, trees=[], params=None):
        self.trees = trees
...
class Modify:
    ...
    specific.
    """
    def __init__(self, modifier, tree, params=None):
        self.mod = modifier
        self.tree = tree
...
def dur(x):
    ...
    if (x.__class__.__name__ == 'Music'):
        ...
        return sum(listMap(dur,x.trees))
    elif (x.__class__.__name__ == 'Par'):
        return max(listMap(dur,x.trees))
...
def mMap(f, x):
    ...
    elif (x.__class__.__name__ == 'Seq' or x.__class__.__name__ == 'Par'):
        for t in x.trees:
            mMap(f, t)
    elif (x.__class__.__name__ == 'Modify'):
        mMap(f, x.tree)
...
def reverse(x):
    def reverseInPlace(x):
        ...
        if (x.__class__.__name__ == 'Music'):
            ...
            newTrees = []
            for t in x.trees:
                newTrees.append(Seq[Rest(dMax - dur(t)), reverse(t)])
            x.trees = newTrees
        elif (x.__class__.__name__ == 'Modify'):
            reverse(x.tree)
        else: raise EuterpeaException("Unrecognized musical structure: "+str(x))
    return reverseInPlace(deepcopy (x))
...
def mFold(x, noteOp, restOp, seqOp, parOp, modOp):
    ...
    if (x.__class__.__name__ == 'Music'):
        ...
        vals = listMap(lambda t: mFold(t, noteOp, restOp, seqOp, parOp, modOp), x.trees)
        return seqOp(vals)
    elif (x.__class__.__name__ == 'Par'):
        vals = listMap(lambda t: mFold(t, noteOp, restOp, seqOp, parOp, modOp), x.trees)
        return parOp(vals)
...
def firstPitch(x):
    ...
    if (x.__class__.__name__ == 'Music'):
        ...
        vals = listMap(firstPitch, x.trees)
        if len(vals) > 0:
            return vals[0]
        else:
            return -1
...
def removeInstruments(x):
    ...
    if x.__class__.__name__ == 'Music':
        ...
        x.trees = listMap(removeInstruments, newTrees)
        return x
    elif x.__class__.__name__ == 'Modify':
        xNew = checkInstMod(x)
        return xNew
...
def onOffToRelDur(evs):
    ...
    :return:
    """
    durs = [0] + listMap(lambda e: e.eTime, evs)
    for i in range(0, len(evs)):
        evs[i].eTime -= durs[i]


def eventPatchList(mevs):
    patches = listMap(lambda e: e.patch, mevs) # extract just the patch from each note
...
def mEventsToPattern(mevs):
    ...
    usedChannels = listMap(lambda p: p[1], pmap) # which channels are we using? (Important for drum track)
...
def pitchListToMusic(ps, defDur=0.25, defVol=100):
    ns = listMap(lambda p: pitchToNote(p, defDur, defVol), ps)
    return line(ns)
...
def pdPairsToMusic(pds, defVol=100):
    ...
    """
    ns = listMap(lambda x: Note(x[0], x[1], defVol), pds)
    return line(ns)
...
def pitchListToChord(ps, defDur=0.25, defVol=100):
    if ps == None:
        ...
        ns = listMap(lambda p: Note(p, defDur, defVol), ps)
        return chord(ns)
...
def chordListToMusic(chords, defDur=0.25, defVol=100):
    cList = listMap(lambda x: pitchListToChord(x, defDur, defVol), chords)
    return line(cList)
