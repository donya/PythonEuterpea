...
class Chunk:
    ...
    def __init__(self, kind, value):
        ...
        if (kind=="R"): # value is a tuple of onset and duration
            ...
            if (len(self.value) > 0):
                ...
                self.dur = sum (listMap(lambda x: x.dur, self.value))
            ...
            if (len(self.value) > 0):
                self.onset = min(listMap(lambda x:x.onset, self.value))
                self.end = min(listMap(lambda x:x.end, self.value))
    ...
    def toMusic(self):
        if (self.kind=="R"):
            ...
            return line(listMap(lambda x: x.toMusic(), self.value))
        elif (self.kind=="Chord"):
            return chord(listMap(lambda x: x.toMusic(), self.value))
        elif (self.kind=="Par"):
            return chord(listMap(lambda x: line([Rest(x.onset), x.toMusic()]), self.value))
        else:
