...
def dur(x):
    ...
    if (x.__class__.__name__ == 'Music'):
        ...
        return sum(map(dur,x.trees))
    elif (x.__class__.__name__ == 'Par'):
        return max(map(dur,x.trees))
...
def mFold(x, noteOp, restOp, seqOp, parOp, modOp):
    ...
    if (x.__class__.__name__ == 'Music'):
        ...
        vals = map(lambda t: mFold(t, noteOp, restOp, seqOp, parOp, modOp), x.trees)
        return seqOp(vals)
    elif (x.__class__.__name__ == 'Par'):
        vals = map(lambda t: mFold(t, noteOp, restOp, seqOp, parOp, modOp), x.trees)
        return parOp(vals)
...
def removeInstruments(x):
    ...
    if x.__class__.__name__ == 'Music':
        ...
        x.trees = map(removeInstruments, newTrees)
        return x
    elif x.__class__.__name__ == 'Modify':
        xNew = checkInstMod(x)
        return xNew
